...
for (i in 1:n) {
  ...
  deltas[i] = DeltaStraddle(sd1)
  count = round(deltas[i] * optCount)
  counts[i] = count
  
  if (i == 1) {
    count = round(deltas[i] * optCount)
    balance[i] = CallClose[i] * optCount + PutClose[i] * optCount - count * Close[i]
  }
  else {
...
profit = balance[n] - CallClose[n] * optCount - PutClose[n] * optCount + deltas[n] * Close[n]
print(profit)
