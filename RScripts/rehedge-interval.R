...
n = length(asset) # number of trading hours
...
for (i in 1:(n-1)) {
  ...
  if (i == 1) {
    ...
    if (callStrikes[1] < asset[i]) {
      # loss on call
      callLosses[i] = (asset[i] - callStrikes[1]) * count
    }
    else
      callLosses[i] = 0
    
    if (putStrikes[1] > asset[i]) {
      # loss on put
      putLosses[i] = (putStrikes[1] - asset[i]) * count
    }
    else 
      putLosses[i] = 0
    
    #if (abs(futures[i] - futures[i - 1]) > 1) {
      money[i] = money[i - 1] - ((futures[i] - futures[i - 1]) * asset[i]) #- callLosses[i] - putLosses[i]
      portfolio[i] = money[i] - callLosses[i] - putLosses[i] + futures[i] * asset[i]
    #}
    #else {
    #  money[i] = money[i - 1]
    #}
...
if (callStrikes[1] < asset[n]) {
  # loss on call
  callLoss = (asset[n] - callStrikes[1]) * count
}

if (putStrikes[1] > asset[n]) {
  # loss on put
  putLoss = (putStrikes[1] - asset[n]) * count
}

money[n] = money[n - 1] + futures[n-1] * asset[n] - putLoss - callLoss
